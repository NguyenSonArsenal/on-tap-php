...
function eventHandleAddUser()
{
    ...
    var testEmail = emailPattern.test(email);
...
function makeHtmlUser(user) {
    ...
    var str =
        ...
        '</td>' +
        '<td>' +
            '<a class="btn btn-primary btn-xs" onclick="clickEditUser(this)">' +
...
// redirect to form edit to edit a user
...
// handle action edit a user from form edit user
function handleEditUser()
{
    ...
    var errors = {
        'name': 'Username must be 6 to 24 characters string',
        'email': 'Email not is invalid xxx',
        'password': 'Password must be 6 to 24 characters string',
        'confirm_password': 'Conform password is not match password',
        'gender': 'Please choose a gender',
    };
...
function searchUser() {
    var nodeIputTextToSearchElement = document.getElementById('keyToSearch');

    var keyToSearch = (nodeIputTextToSearchElement.value).trim();

    if(keyToSearch.indexOf('@') > 0)
    {
       searchByEmail(keyToSearch);
    }
    else
    {
        searchByName(keyToSearch);
    }
}
...
function searchByName(needle)
{
    ...
    if (length > 0)
    {
        for (var i=0; i< length; i++)
        {
            arrNameUser.push(users[i].name);
        }

        for (var i=0; i<length; i++)
        {
            var name = changeToSlug(users[i].name);

            needle = changeToSlug(needle);

            if (name.indexOf(needle) >= 0)
            {
                arrUserFound.push(users[i]);
            }
        }
        var parentNodeuser = document.getElementById('tbodyUser');
        parentNodeuser.innerHTML='';
        fetchUserFoundBySearch(arrUserFound);
    }
}

function searchByEmail(needle)
{
    var users = JSON.parse(getItemInLocalStorage('module_user')) || [];
    ...
    if (length > 0)
    {
        for (var i=0; i< length; i++)
        {
            arrEmailUser.push(users[i].email);
        }

        for (var i=0; i<length; i++)
        {
            var email = changeToSlug(users[i].email);
            needle = changeToSlug(needle);

            if (users[i].email == needle)
            {
                arrUserFound.push(users[i]);
            }
        }
...
// show user found by search
function fetchUserFoundBySearch(users) {
    var parentNodeuser =  document.getElementById('tbodyUser');
    parentNodeuser.innerHTML='';
